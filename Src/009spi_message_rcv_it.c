 ...
 *
 * Board Connections
 * STM32F407xx --> ArduinoUno
 * PB14 --> SPI2_MISO --> PIN 12
 * PB15 --> SPI2_MOSI --> PIN 11
 * PB13 --> SPI2_SCLK --> PIN 13
 * PB12 --> SPI2_NSS  --> PIN 10
 * ALT function mode : 5
 * PD6  --> SLAVE INT --> PIN 8
 */
#include<stdio.h>
...
void Slave_GPIO_InterruptPinInit(void)
{
	...
	GPIO_IRQPriorityConfig(IRQ_NO_EXTI5_9,NVIC_IRQ_PRIORITY5);
...
int main(void)
{
	printf("009spi_message_rcv.it Application Started!\n");
	uint8_t dummy = 0xff;
	...
	*/
	...
	while(1){
		...
		GPIO_IRQITConfig(IRQ_NO_EXTI5_9,DISABLE);
		...
		while(!rcvStop)
		{
			...
			while ( SPI_DataSendIT(&SPI2handle,&dummy,1) == SPI_BUSY_IN_TX);
			while ( SPI_DataReceiveIT(&SPI2handle,&ReadByte,1) == SPI_BUSY_IN_RX );
		}
